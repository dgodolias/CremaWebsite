...
function shouldUseStaticPresentation() {
  return (
...
function subscribeToStaticPresentation(onStoreChange: () => void) {
  const mediaQuery = window.matchMedia(STATIC_PRESENTATION_QUERY);
  mediaQuery.addEventListener("change", onStoreChange);
  return () => mediaQuery.removeEventListener("change", onStoreChange);
}

function useStaticPresentation() {
  return useSyncExternalStore(
    subscribeToStaticPresentation,
...
export interface PixelLiquidBgProps extends ComponentProps<"div"> {
  ...
  autoDemo?: boolean;
  /** Skip WebGL and render the palette-derived static surface. */
  forceStatic?: boolean;
  /** Additional class name for the static fallback surface. */
...
export function PixelLiquidBg({
  ...
  const mediaRequiresStatic = useStaticPresentation();
  const [webglFailed, setWebglFailed] = useState(false);
  const useStaticFallback =
    forceStatic || mediaRequiresStatic || webglFailed;
  const normalizedDarkPalette =
    darkPalette.length > 0 ? darkPalette : DEFAULT_DARK_PALETTE;
  const normalizedLightPalette =
    lightPalette.length > 0 ? lightPalette : DEFAULT_LIGHT_PALETTE;

  useEffect(() => {
    if (useStaticFallback || shouldUseStaticPresentation()) return;
    ...
    const palette = makePaletteTexture(
      dark ? normalizedDarkPalette : normalizedLightPalette,
    );
  ...
  return (
    <div
      ...
      data-renderer={useStaticFallback ? "static" : "webgl"}
      {...props}
    >
      {useStaticFallback && (
        <>
