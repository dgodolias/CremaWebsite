...
- **Photo policy:** Existing food sections continue to use authentic menu/vendor photography. The two images in the owner-approved interactive crepe explorer are the sole AI-generated exception and must be recorded as generated assets.
...
**24/7 discovery and footer amendment — approved by the owner on 2026-09-11:** Keep the opening-hours claim visible only once, in the existing location badge. Lead the page metadata with the owner-provided `24 ώρες / 7 ημέρες` fact and expose it through static `CafeOrCoffeeShop` JSON-LD, while keeping the body free of duplicate hours copy. The footer adds one compact Instagram link using the recognizable gradient camera mark and only `@crema_gazi` as its visible profile text; it must remain keyboard accessible and reflow cleanly on mobile.

**Interactive crepe explorer amendment — approved by the owner on 2026-09-11:** Remove the standalone Provio spotlight and replace it, immediately after the orange category band beneath the hero, with a compact two-card `Φτιάξε την κρέπα σου` section. The savory and sweet crepe cards use new owner-approved AI-generated food photographs and the slogan `Κρέπες με αγνά υλικά καθημερινά, μείγμα δικό μας και κάθε δημιουργία φτιαγμένη με αγάπη, φροντίδα και μεράκι. Γιατί η καλύτερη γεύση ξεκινά από όσα διαλέγουμε και ολοκληρώνεται σε κάθε ζεστή, λαχταριστή μπουκιά.` Desktop pointer hover and keyboard focus reveal a categorized, internally scrollable ingredient panel; touch users toggle the same panel with a tap and close it with a second tap. Ingredient prices are excluded. The experience uses clear expanded state, visible focus, Escape dismissal, reduced-motion-safe transitions and mobile reflow. Its visual direction is a focused `Balanced` moment inside the otherwise restrained page: dark Crema framing, cream menu surfaces, green/orange card accents, editorial `01/02` numbering and a compact custom scrollbar. Generated prompts, provenance and optimized AVIF outputs are recorded with the project assets.

**Logo-led hero amendment — revised and approved by the owner on 2026-09-10:** The hero's green Dimello/delivery kicker and the text-rendered handwritten `Crema` line are removed. The existing official Crema logo becomes the first and dominant hero read in their place. `Νέες γεύσεις / στο Γκάζι` remains directly below it as the quieter two-line H1 support, followed by the existing description and order action. The logo's alternative text preserves `Crema` in the accessible heading name.

**Calm visual-system amendment — approved 2026-09-10 (Option A):** The page moves from `Balanced` to `Restrained`. Large supporting typography—including the category band, section titles, `Provio`, story statement and delivery numeral—is reduced by at least 25–35%, with former extra-large marks reduced further so the official Crema logo remains the sole dominant brand moment. Section spacing increases, accent colors become softer and less saturated, body copy gains a gentler reading rhythm, and hover/scroll effects use lower movement, zoom, blur and saturation. Primary actions remain clearly identifiable and retain accessible target sizes.
...
Selected expression: `Restrained`; the selected amplifiers are X01 restrained editorial composition supporting the official logo, X02 signature imagery, a reduced-amplitude scroll-linked hero and gentle one-time entrance/reveal transitions. Third-party smooth scrolling, continuous animation loops, custom-cursor and marquee animation remain excluded to protect calmness and runtime performance.
