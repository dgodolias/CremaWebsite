...
**Hero hierarchy amendment — approved 2026-09-10 (Option A):** `Crema` is the first and dominant visual read in the homepage hero, set as the largest handwritten display line. `Νέες γεύσεις / στο Γκάζι` remains part of the semantic H1 but is rendered at roughly 30% of the brand scale, with quieter weight, spacing and contrast. This amendment preserves the existing copy and palette while reducing the headline's overall visual volume on both desktop and mobile.

**Calm visual-system amendment — approved 2026-09-10 (Option A):** The page moves from `Balanced` to `Restrained`. Large supporting typography—including the category band, section titles, `Provio`, story statement and delivery numeral—is reduced by at least 25–35%, with former extra-large marks reduced further so `Crema` remains the sole oversized brand moment. Section spacing increases, accent colors become softer and less saturated, body copy gains a gentler reading rhythm, and hover/scroll effects use lower movement, zoom, blur and saturation. Primary actions remain clearly identifiable and retain accessible target sizes.
...
Selected expression: `Restrained`; the selected amplifiers are X01 expressive typography limited to the brand-first `Crema` moment, X02 signature imagery, a reduced-amplitude scroll-linked hero and gentle one-time entrance/reveal transitions. Third-party smooth scrolling, continuous animation loops, custom-cursor and marquee animation remain excluded to protect calmness and runtime performance.
